...
class GetCurrentStateDeltaMembershipChangesForUserFederationTestCase(
    ...
    def test_remote_join(self) -> None:
        ...
        creation_requester = create_requester(f"@creator:{self.OTHER_SERVER_NAME}")
        creation_event_source = {
            ...
            "origin_server_ts": 0,
            "prev_events": [],
            "sender": creation_requester.user.to_string(),
            "state_key": "",
            "type": EventTypes.Create,
        }
        if not self.room_version.msc4291_room_ids_as_hashes:
            # We will pull this out on the other side of the creation event generation
            # to use for the rest of the test, in case it is generated by the event
            # hash. There seems no sense in saving it early.
            creation_event_source["room_id"] = f"!example:{self.OTHER_SERVER_NAME}"
        if not self.room_version.implicit_room_creator:
            content_dict = creation_event_source["content"]
            # mypy seems to have issues deducing that "content" is a dict, give it a
            # hand.
            assert isinstance(content_dict, dict)
            content_dict.update({"creator": creation_requester.user.to_string()})
        ...
        # is a msc4921 room
        creation_event = make_event_from_dict(
            creation_event_source,
            self.room_version,
            {},
            None,
        ...
        if not self.room_version.msc4291_room_ids_as_hashes:
            creator_join_event_source["auth_events"] = [creation_event.event_id]
        self.add_hashes_and_signatures_from_other_server(
            creator_join_event_source,
            self.room_version,
        )
        ...
        if not self.room_version.msc4291_room_ids_as_hashes:
            join_event_source["auth_events"] = [creation_event.event_id]
